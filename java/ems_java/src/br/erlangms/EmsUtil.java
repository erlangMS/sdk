...
public final class EmsUtil {
	...
	public static final OtpErlangBinary erro_convert_json = new OtpErlangBinary(
			"{\"erro\":\"service\", \"message\" : \"Falha na serialização do conteúdo em JSON\"}".getBytes());
	...
	static {
		...
		gson = new GsonBuilder().setExclusionStrategies(new SerializeStrategy()).setDateFormat("dd/MM/yyyy")
		...
		gson2 = new GsonBuilder().setExclusionStrategies(new SerializeStrategy()).setDateFormat("dd/MM/yyyy")
				// .serializeNulls() <-- uncomment to serialize NULL fields as well
				.registerTypeAdapter(BigDecimal.class, new JsonSerializer<BigDecimal>() {
					@Override
					public JsonElement serialize(BigDecimal value, Type arg1,
	...
	public static JsonObject getMethodRemote(String pathCatalog, String method, String token) {
		try {
			URL url = new URL(pathCatalog);
	...
	private static class SerializeStrategy implements ExclusionStrategy {
		...
		public boolean shouldSkipField(FieldAttributes f) {
			...
			return (f.getDeclaredType() == List.class || f.getAnnotation(OneToMany.class) != null
					|| f.getAnnotation(JoinTable.class) != null || f.getAnnotation(ManyToMany.class) != null);
		}
	}
	 ...
	 * @param obj
	 *            Objeto que será serializado para json
	 * @param serializeFullObject
	 *            Se true, serializa atributos de classe também
	 * @return string json da serialização
	 * @author Everton de Vargas Agilar
	 */
	...
	/**
	 * Serializa um objeto a partir de uma string json. Quando a string json é
	 ...
	 * @param classOfObj
	 *            Classe do objeto que será serializado
	 * @param jsonModelAdapter
	 *            adaptador para permitir obter atributos de modelo
	 * @return objeto
	 * @author Everton de Vargas Agilar
	 */
	@SuppressWarnings("unchecked")
			...
			try {
				if (jsonString != null && !jsonString.isEmpty()) {
					if (classOfObj == List.class || classOfObj == ArrayList.class) {
						...
						} catch (InstantiationException | IllegalAccessException | EmsValidationException
								| InvocationTargetException | NoSuchMethodException | SecurityException e) {
							throw new EmsValidationException("Não suporta conversão do json da classe "
									+ classOfObj.getSimpleName() + ". Json: " + jsonString);
						}
				...
				throw new EmsValidationException(
						"Não suporta instânciar objeto para a classe " + classOfObj.getSimpleName());
			}
		} else {
	...
	/**
	 * Obtém uma lista a partir de um json. Se o json estiver vazio, retorna apenas
	 * uma lista vazia.
	 * 
	 * @param jsonString
	 *            String json
			...
			if (jsonString != null && !jsonString.isEmpty()) {
				...
				for (Object value : values) {
					try {
						...
						throw new EmsValidationException("Não suporta conversão do json para "
								+ classOfObj.getSimpleName() + ". Json: " + jsonString);
					}
				}
			}
	 ...
	 * 
	 * @param query
	 *            Instância da query com parâmetros a setar
	 * @param values
	 *            Map com chave/valor dos dados que serão aplicados na query
	 * @author Everton de Vargas Agilar
	 */
	@SuppressWarnings({ "rawtypes", "unchecked" })
		...
		if (query != null && values != null && values.size() > 0) {
			...
			for (String field : values.keySet()) {
				try {
					...
					throw new EmsValidationException(
							"Erro ao setar parâmetros da query. Erro interno: " + e.getMessage());
				}
			}
		}
	}
	...
	/**
	 * Passando um objeto e retorna um map. Se o obj já é um map não faz nada e
	 * apenas o retorna.
	 * 
	 * @param obj
	 ...
	 *            Instância de um objeto
	 * @param values
	 *            Map com chave/valor dos dados que serão aplicados no objeto
	 * @param jsonModelAdapter
	 *            jsonModelAdapter
	 ...
	 */
	@SuppressWarnings({ "rawtypes", "unchecked" })
	public static Object setValuesFromMap(final Object obj, final Map<String, Object> values,
			final EmsJsonModelAdapter jsonModelAdapter) {
			...
			for (String field_name : values.keySet()) {
				try {
					...
					if (tipo_field == Integer.class || tipo_field == int.class) {
						...
						if (new_value instanceof String) {
							...
							try {
								if (len_value == 0) {
									...
									field.set(obj,
											new java.sql.Date(dateFormatDDMMYYYY.parse((String) new_value).getTime()));
								} else if (len_value == 16) {
						...
						field.set(obj, value);
					} else if (tipo_field instanceof Object && findFieldByAnnotation(tipo_field, Id.class) != null) {
						try {
							Integer idValue = null;
							if (new_value instanceof String) {
								idValue = Integer.parseInt((String) new_value);
					...
					throw new EmsValidationException(
							"Campo " + field_name + " inválido. Erro interno: " + e.getMessage());
				}
			}
		}
		return obj;
	 ...
	 * 
	 * @param clazz
	 *            Classe pojo. Ex.: OrgaoInterno.class
	 * @param ann
	 *            anotação que será pesquisada. Ex.: Id.class
	 ...
	 * 
	 * @param value
	 *            código da enumeração
	 * @param clazz
	 *            classe da enumeração
	 * @return enumeração
	 * @author Everton de Vargas Agilar
	 */
	...
	public static Enum<?> StrToEnum(final String value, @SuppressWarnings("rawtypes") final Class<Enum> clazz) {
		if (value != null && !value.isEmpty() && clazz != null) {
			...
			throw new IllegalArgumentException(
					"Parâmetros clazz e value do método EmsUtil.StrToEnum não devem ser null.");
		}
	}
	...
	/**
	 * Converte um objeto Java para um oobjeto de response. Isso é utilizado para
	 * enviar a resposta para o barramento no formato nativo Erlang.
	 * 
	 * @param ret
	 *            objeto
	 * @param request
	...
	public static OtpErlangTuple serializeObjectToErlangResponse(final Object ret, final IEmsRequest request) {
		...
		if (ret != null) {
			try {
				...
				} else if (ret instanceof java.util.Date || ret instanceof java.sql.Timestamp
	...
	public static boolean isDateFinalAfterOrEqualDateInitial(final Date dataIni, final Date dataFinal) {
		return (dataFinal != null && dataIni != null && (dataFinal.equals(dataIni) || dataFinal.after(dataIni)) ? true
				: false);
	}
	...
	public static String listFunctionToSqlFunction(final List<String> listFunction) {
		if (listFunction != null) {
			if (listFunction.isEmpty() || (listFunction.size() != 2)) {
				throw new EmsValidationException(
						"Função SQL precisa de um operador e de uma coluna para EmsUtil.listFunctionToSqlFunction.");
			}
			...
			throw new EmsValidationException(
					"Função SQL " + function + " inválido para EmsUtil.listFunctionToSqlFunction.");
		} else {
			throw new EmsValidationException(
					"Parâmetro listFunction não pode ser null para EmsUtil.listFunctionToSqlFunction.");
		}
	}
	 ...
	 * 
	 * @param value_field
	 *            valor String, Double ou Boolean.
	 * @return boolean
	...
	/**
	 * Gera um relatório no formato pdf a partir de um template jasper. É importante
	 * o contrato do serviço declarar "content_type" : "application/pdf" para que o
	 * browser exiba o pdf corretamente.
	 * 
	 * @param params
	 *            parâmetros do relatório.
	 * @param datasource
	 *            é o objeto ou lista de objetos do relatório. Pode ser null.
	 * @param templateJasper
	 *            arquivo .jasper do template criado no Ireport. É obrigatório. Ex.:
	 *            "/relatorios/DeclaracaoAlunoRegular.jasper"
	 * @param owner
	 *            referência para objeto quem invoca esse metodo. Utilizado para
	 *            owner.getClass().getResourceAsStream()
	 * @return byte[] retorna um fluxo de bytes que deve ser tratado pelo frontend
	 *         para geração do pdf
	 * @author Fabiano Rodrigues de Paiva
	 * @author Everton de Vargas Agilar (revisão)
	 */
	@SuppressWarnings("unchecked")
	public static byte[] printPdf(final Object params, final Object datasource, final String templateJasper,
			final Object owner) {
		if ((templateJasper != null && !templateJasper.isEmpty()) && owner != null) {
			try {
				Map<String, Object> paramsMap = null;
				List<Object> objectList = null;
				InputStream streamTemplateJasper = owner.getClass().getResourceAsStream(templateJasper); // com ajuda do
																											// owner
																											// recupero
																											// o caminho
																											// onde está
				...
				throw new EmsValidationException(
						"Não foi possível gerar o pdf pois as fontes utilizadas não foram encontradas no servidor. Erro interno: "
								+ e.getLocalizedMessage());
			} catch (Exception e) {
				e.printStackTrace();
				throw new EmsValidationException(
						"Não foi possível gerar o pdf pois um erro interno ocorreu: " + e.getLocalizedMessage());
			}
		} else {
			throw new EmsValidationException(
					"Parâmetros params ou listaObj, templateJasper e owner devem ser informados para EmsUtil.printPdf");
		}
	}
	...
	public static UniqueConstraint[] getTableUniqueConstraints(final Class<?> classOfModel) {
		if (classOfModel != null) {
			...
			throw new EmsValidationException(
					"Parâmetro classOfModel não pode ser null para EmsUtil.getTableUniqueConstraints.");
		}
	}
	...
	public static List<Field> getFieldsWithUniqueConstraint(final Class<?> classOfModel) {
		if (classOfModel != null) {
			...
			throw new EmsValidationException(
					"Parâmetro classOfModel não pode ser null para EmsUtil.getFieldsWithUniqueConstraint.");
		}
	}

	/**
	 * Obter a lista de fields de um model. Obs.: somente fields com a anotação
	 * Column são retornados.
	 * 
	 * @param classOfModel
	 *            classe do modelo
	 * @return lista de campos
	...
	public static List<Field> getFieldsFromModel(final Class<?> classOfModel) {
		if (classOfModel != null) {
			...
			throw new EmsValidationException(
					"Parâmetro classOfModel não pode ser null para EmsUtil.getFieldsFromModel.");
		}
	}
	 ...
	 * 
	 * O processo ems_ldap_server precisa estar habilitado no barramento, caso ele
	 * seja o servidor LDAP.
	 * 
	 * As seguintes informações são lidas das propriedades. Verifique
	 * EmsUtil.properties. -Dems_ldap_admin_passwd="123456"
	 * -Dems_ldap_admin="cn=admin,dc=unb,dc=br"
	 * -Dems_ldap_url="ldap://localhost:2389"
	 * 
	 * Obs.: As properties devem ser incluídas no arquivo standalone.conf do
	 * JBoss/Wildfly ou na IDE Eclipse Open Launch Configuration/VM Arguments
	 * 
	 * @param login
	 *            do usuário. Ex: geral
	...
	public static Object ldapSearch(final String login) {
		if (login != null) {
			...
			env.put(Context.SECURITY_CREDENTIALS, properties.ldapAdminPasswd); // admin password in ems_ldap_server.json
																				// catalog
			...
			try {
				...
				throw new EmsValidationException(
						"Não foi possível pesquisar usuário no servidor LDAP " + properties.ldapUrl);
			}
		} else {
	 ...
	 * 
	 * Para que seja posśivel a conversão é necessário passar a lista dos campos
	 * (fields).
	 * 
	 * @param fields
	 *            lista de campos. Pode ser passado como um array de campos, string
	 *            de campos separado por vírgula ou lista de campos.
	 * @param listObj
	 *            lista de objetos
	...
	public static List<Map<String, Object>> ListObjectToListMap(final Object fields, final List<?> listObj) {
		if (fields != null && listObj != null) {
			...
			if (fields instanceof String) {
				...
				throw new EmsValidationException(
						"Parâmetro fields não é do tipo correto para EmsUtil.ListObjectToListMap.");
			}
			...
			throw new EmsValidationException(
					"Parâmetros fields e ListObj não podem ser null para EmsUtil.ListObjectToListMap.");
		}
	}
	 ...
	 * 
	 * Essas informações são carregadas durante durante o deployment do arquivo
	 * standalone.conf no JBoss/Wildfly.
	 * 
	 * No servidor JBoss/Wildfly, estão definidas em
	 * configuration/standalone/standalone.conf. As properties podem ser incluídas
	 * também na IDE Eclipse Open Launch Configuration/VM Arguments
	 *
	 * 
	 * Definições de propriedade no JBoss/Widfly: -Dcookie=erlangms
	 * -Dems_node=node01 -Dems_emsbus=http://localhost:2301 -Dems_cookie=erlangms
	 * -Dems_max_thread_pool_by_agent=100 -Dems_user=xxxxxxx
	 * -Dems_ldap_admin_passwd="xxxxxxx" -Dems_ldap_admin="cn=admin,dc=unb,dc=br"
	 * -Dems_ldap_url="ldap://localhost:2389" -Dems_smtp_passwd="xxxxxxxx"
	 * -Dems_smtp_from="erlangms@unb.br" -Dems_smtp_port=587
	 ...
	 * 
	 * Exemplo: -Dems_node=node01 -Dems_emsbus_url=http://localhost:2301
	 * -Dems_emsbus=ems_bus -Dems_cookie=erlangms -Dems_max_thread_pool_by_agent=100
	 * -Dems_debug=false -Dems_user=xxxxxxx -Dems_ldap_admin_passwd="xxxxxx"
	 * -Dems_ldap_admin="cn=admin,dc=unb,dc=br"
	 * -Dems_ldap_url="ldap://localhost:2389" -Dems_smtp_passwd="xxxxxxxx"
	 * -Dems_smtp_from="erlangms@unb.br" -Dems_smtp_port=587
	 * -Dems_smtp="mail.unb.br"
	...
	private static EmsProperties getProperties() {
		...
		// Atenção: ems_daemon_params deve ser o primeiro parâmetro lido das
		// propriedades
		// pois os próximos parâmetros podem ser armazenados em ems_daemon_params ou
		// obtidos das properties da JVM
		...
		if (tmp_daemon_params != null) {
			try {
				...
				System.out.println("Não foi possível fazer o parse do parâmetro ems_daemon_params. Erro interno: "
						+ e.getMessage());
	 ...
	 * 
	 * Exemplo sem anexo: EmsUtil.sendTextMail("evertonagilar@gmail.com", "isso é um
	 * teste", "conteúdo do email", null);
	 * 
	 * Exemplo com anexo: String[] anexos = new String[1]; anexos[0] =
	 * "/home/everton/desenvolvimento/erlangms/ems-bus/build.sh";
	 * EmsUtil.sendTextMail("evertonagilar@gmail.com", "isso é um teste", "conteúdo
	 * do email", anexos);
	 *
	 * 
	 * As seguintes informações são lidas das properties. Verifique
	 * EmsUtil.properties: -Dems_smtp_passwd="xxxxxxxx"
	 * -Dems_smtp_from="erlangms@unb.br" -Dems_smtp_port=587
	 * -Dems_smtp="mail.unb.br"
	 * 
	 * Obs.: As properties devem ser incluídas no arquivo standalone.conf do
	 ...
	 * @param attachment
	 *            lista de arquivos para anexar do e-mail. (Opcional)
	 * @author Everton de Vargas Agilar
	 */
	public static void sendTextMail(final String to, final String subject, final String content,
		...
		try {
			...
			throw new EmsValidationException(
					"Não foi possível enviar e-mail para " + to + ". Erro interno: " + ex.getMessage());
		}
	}

	/**
	 * Permite enviar um e-mail em formato HTML com uma lista de anexos. (private
	...
	public static void sendHtmlMail(final String to, final String subject, final String content,
			final String[] attachment) {
		MailSenderInfo mailInfo = new MailSenderInfo();
		mailInfo.setMailServerHost(properties.smtp);
		mailInfo.setMailServerPort(Integer.toString(properties.smtpPort));
	 ...
	 * retorna null. Se o filter estiver com sintáxe incorreta, retorna exception
	 * EmsValidationException.
	 * 
	 * @param filter
	 *            filtro. Ex.: {"nome":"Everton de Vargas Agilar", "ativo":true}
	...
	//
	// Executa um comando e imprime sua saída no terminal se imprimeSaida for true
	// (apenas Linux).
	//
	//
	...
	//
	// Captura o sinal enviado por kill para finalizar o processo e faça o
	// encerramento corretamente.
	// Agilar: Para ctrl + c ou kill [processo]. O poderoso kill -9 não é capturado,
	// portanto seja legal e não use o parâmetro -9.
	//
	...
	public static String getProperty(final String p) {
		if (p != null && !p.isEmpty()) {
			...
			if (properties != null) {
				...
				if ((p.equals("erlangms.java_thread_pool") || p.equals("ems_thread_pool"))
						&& c.containsKey("erlangms.java_thread_pool")) {
				...
				if ((p.equals("erlangms.smtp.password") || p.equals("ems_smtp_passwd"))
						&& c.containsKey("erlangms.smtp.password")) {
					return (String) c.get("erlangms.smtp.password");
				}

				// Parâmetro erlangms.smtp.from
				if ((p.equals("erlangms.smtp.from") || p.equals("ems_smtp_from"))
						&& c.containsKey("erlangms.smtp.from")) {
					return (String) c.get("erlangms.smtp.from");
				}

				// Parâmetro erlangms.smtp.port
				if ((p.equals("erlangms.smtp.port") || p.equals("ems_smtp_port"))
						&& c.containsKey("erlangms.smtp.port")) {
				...
				if ((p.equals("erlangms.environment") || p.equals("ems_environment"))
						&& c.containsKey("erlangms.environment")) {
				...
				if ((p.equals("erlangms.max_thread_pool_by_agent") || p.equals("ems_max_thread_pool_by_agent"))
						&& c.containsKey("erlangms.max_thread_pool_by_agent")) {
				...
				if ((p.equals("erlangms.ldap.passwd") || p.equals("ems_ldap_admin_passwd"))
						&& c.containsKey("erlangms.ldap.passwd")) {
					return (String) c.get("erlangms.ldap.passwd");
				}

				// Parâmetro erlangms.ldap.admin
				if ((p.equals("erlangms.ldap.admin") || p.equals("ems_ldap_admin"))
						&& c.containsKey("erlangms.ldap.admin")) {
				...
				if ((p.equals("erlangms.msg_timeout") || p.equals("ems_msg_timeout"))
						&& c.containsKey("erlangms.msg_timeout")) {
					return (String) c.get("erlangms.msg_timeout").toString();
				}

				// Parâmetro erlangms.post_update_timeout
				if ((p.equals("erlangms.post_update_timeout") || p.equals("ems_post_update_timeout"))
						&& c.containsKey("erlangms.post_update_timeout")) {
					return (String) c.get("erlangms.post_update_timeout").toString();
				}

				// Parâmetro erlangms.daemon_id
				if ((p.equals("erlangms.daemon_id") || p.equals("ems_daemon_id"))
						&& c.containsKey("erlangms.daemon_id")) {
					return (String) c.get("erlangms.daemon_id");
				}

				// Parâmetro erlangms.daemon_service
				if ((p.equals("erlangms.daemon_service") || p.equals("ems_daemon_service"))
						&& c.containsKey("erlangms.daemon_service")) {
				...
				if ((p.equals("erlangms.pidfile_watchdog_timer") || p.equals("ems_pidfile_watchdog_timer"))
						&& c.containsKey("erlangms.pidfile_watchdog_timer")) {
				...
				if ((p.equals("erlangms.service_scan") || p.equals("ems_service_scan"))
						&& c.containsKey("erlangms.service_scan")) {
	...
	/**
	 * Obtém um parâmetro de configuração como inteiro. Se não encontrar retorna
	 * null
	 * 
	 * @param property
	 *            propriedade
	...
	/**
	 * Obtém um parâmetro de configuração como inteiro ou defaultValue se não
	 * encontrar.
	 * 
	 * @param property
	 *            propriedade
	...
	/**
	 * Permite passar args que recebido em main para o SDK É utilizado para buscar
	 * parâmetros com getProperty
	 * 
	 * @param args
	 *            argumentos
	 * @author Everton de Vargas Agilar
	 ...
	 * 
	 * @param fileNamePid
	 *            nome do arquivo de pid
	 * @param deleteIfExists
	 *            apaga o arquivo se existe
	 * @throws Exception
	 *             retorna erro se não consegue criar o pid
	...
	public static String createPidFile(final String fileNamePid, boolean deleteIfExists) throws Exception {
		if (fileNamePid != null && !fileNamePid.isEmpty()) {
			...
			try (FileWriter fw = new FileWriter(arq)) {
				...
				throw new Exception(
						"Não foi possível criar o arquivo de pid " + fileNamePid + ". Motivo: " + e.getMessage());
			}
		} else {
	 ...
	 * @author Everton de Vargas Agilar, Renato Carauta
	 */
	public static void addUpdatePidFileHook(final String fileNamePid, final EntityManager serviceContext,
			final Integer watchdogTimer) {
					...
					if (isAlive()) {
						try {
							try {
								...
								// Se ficar muito tempo travado o barramento vai matar o processo e subir uma
								// nova instãncia
							...
							logger.info("Atenção: Não foi possível atualizar o arquivo de pid " + fileNamePid
									+ ". Motivo: " + e.getMessage());
						}
					}
				}
		...
		logger.info("Adicionando um hook para atualizar o arquivo de pid " + fileNamePid + " a cada "
				+ String.valueOf(watchdogTimer) + "ms.");
		new UpdatePidFileThead(fileNamePid, serviceContext, watchdogTimer).start();
	}

	/**
	 * Adiciona um hook na JVM para monitorar o arquivo de pid do processo Se ficar
	 * desatualizado por muito tempo, o processo será encerrado normalmente
	 * 
	 * @param fileNamePid
	 *            nome do arquivo do pid
	 * @param watchdogTimer
	 *            tempo em milisegundos
	 * @author Everton de Vargas Agilar, Renato Carauta
	 */
	public static void addMonitorPidFileHook(final String fileNamePid, Integer watchdogTimer) {
					...
					if (isAlive()) {
						try {
							...
							if (diffTempo > watchdogTimer) {
								throw new Exception("O arquivo de pid " + fileNamePid
										+ " está desatualizado há mais de " + String.valueOf(diffTempo) + "ms.");
							}
							...
							if (numeroTentativas > 3) {
								logger.info("Fatal: O processo será encerrado pois está travado. Erro interno: "
										+ e.getMessage());
								removePidFile(fileNamePid);
								System.exit(1);
							} else {
								logger.info("Atenção: O processo parece estar travado (Tentativa: "
										+ String.valueOf(numeroTentativas) + "). Erro interno: " + e.getMessage());
							}
							numeroTentativas++;
						}
					}
		...
		logger.info("Adicionando um hook para monitorar o arquivo de pid " + fileNamePid + " a cada "
				+ String.valueOf(watchdogTimer) + "ms.");
