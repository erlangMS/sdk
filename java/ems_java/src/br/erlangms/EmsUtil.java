...
public final class EmsUtil {
	 ...
	 * 
	 * @param obj              Instância de um objeto
	 * @param values           Map com chave/valor dos dados que serão aplicados no
	 *                         objeto
					...
					if (new_value == null) {
						...
						if (tipo_field == Integer.class || tipo_field == int.class) {
							...
							if (new_value instanceof ArrayList) {
								...
								value = toByteArray(
										new ArrayList(((LinkedTreeMap<Integer, Double>) new_value).values()));
							}
	 ...
	 * Isso é utilizado para enviar a resposta para o barramento no formato nativo
	 * Erlang.
	 * 
	 * @param ret     objeto
		...
		if (ret != null) {
			try {
				...
				if (isEmsResponse) {
					...
					reply[1] = new OtpErlangBinary((byte[]) ret);
				} else if (ret instanceof List) {
					List<?> lista = (List<?>) ret;
					if (lista.isEmpty()) {
						reply[1] = result_list_empty;
					} else {
						OtpErlangObject[] otp_items = new OtpErlangObject[lista.size() * 2 + 1];
						otp_items[0] = new OtpErlangBinary("[".getBytes());
						for (int i = 0; i < lista.size(); i++) {
							String jsonItem = EmsUtil.toJson(lista.get(i));
							if (jsonItem == null)
								jsonItem = "null";
							otp_items[2 * i + 1] = new OtpErlangBinary(jsonItem.getBytes());
							if (i < lista.size() - 1) {
								otp_items[2 * i + 2] = new OtpErlangBinary(",".getBytes());
							} else {
								otp_items[2 * i + 2] = new OtpErlangBinary("]".getBytes());
							}
						}
	 ...
	 * É importante o contrato do serviço declarar "content_type" :
	 * "application/pdf" para que o browser exiba o pdf corretamente.
	 * 
	 * @param params         parâmetros do relatório.
	 * @param datasource     é o objeto ou lista de objetos do relatório. Pode ser
	 *                       null.
	 * @param templateJasper arquivo .jasper do template criado no Ireport. É
	 *                       obrigatório. Ex.:
	 *                       "/relatorios/DeclaracaoAlunoRegular.jasper"
	 * @param owner          referência para objeto quem invoca esse metodo.
	 *                       Utilizado para owner.getClass().getResourceAsStream()
	 ...
	 * configuration/standalone/standalone.conf. As properties
	 * podem ser incluídas também na IDE Eclipse Open Launch Configuration/VM
	 * Arguments
	 *
	 * 
	 ...
	 * Se o filter estiver com sintáxe incorreta, retorna exception
